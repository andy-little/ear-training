...
const SettingsModal = ({ openBtn, location: { left, top, width } }) => {
    ...
    const { notesDispatch, isSelectOpen, setIsSelectOpen /* gameDispatch */ } =
        useEarTrainingContext();
    const modal = useRef(null);
    useClickedOutside(modal, openBtn, () => {
    ...
    return (
        <aside
            ...
            ref={modal}
            className={`select-notes-modal ${isSelectOpen && "show"}`}
        >
            ...
            <div className="select-notes-btn-container">
                <button
                    className="select-notes-btn"
                    onClick={() => {
                        notesDispatch({ type: "SET_ALL" });
                    }}
                >
                    All
                </button>
